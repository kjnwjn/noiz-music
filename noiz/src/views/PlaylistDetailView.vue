        ...
        <table class="table-auto w-full">
          <thead
            class="
              ...
              text-left
              uppercase
              border-b-slate-400 border-solid border-b-2 border-opacity-50
            "
          >
            <tr>
              <th class="py-5">#</th>
          ...
          <tbody v-if="playList">
            <tr
              class="song_item"
              v-for="(song, index) in playList.song.items"
              :key="song.encodeId"
            >
              <td class="stt">
                <p
                  ...
                  v-if="
                    (!isPlaying && !isLoading) ||
                    currentSong.song?.encodeId != song.encodeId
                  "
                >
                  {{ index }}
                </p>
                ...
                >
                  <font-awesome-icon
                    v-if="
                      isLoading && currentSong.song?.encodeId == song.encodeId
                    "
                    class="loading"
...
export default {
  ...
  async mounted() {
    ...
    await axios
      .get(
        `http://localhost:3000/api/v1/playlist/${this.$route.params.id}/detail?api_key=${process.env.VUE_APP_API_KEYS}`
      )
      .then((res) => {
